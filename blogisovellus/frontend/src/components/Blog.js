            ...
            <Panel bsStyle="info">
                <Panel.Heading>
                    <Panel.Title><h2 className="subject">{blog.subject} </h2>{blog.date}
                        <div><em>Tykkäykset: <Badge>{blog.likes}</Badge> | Kirjoittaja: <Link to={`/users/${blog.user._id}`}>{blog.user.name}</Link> | Kategoriat: {blog.categories.map(category => <div key={category._id}> {category._id}</div>)}</em></div></Panel.Title>
                </Panel.Heading>
                <Panel.Body>
                    <div style={hide}>
